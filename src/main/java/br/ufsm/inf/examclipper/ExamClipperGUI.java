...
import br.ufsm.inf.examclipper.gui.FooterPanel;
...
public class ExamClipperGUI extends JFrame {
   ...
   private void initComponents() {
      ...
      add(footerPanel.getPanel(), BorderLayout.PAGE_END);
   ...
   private void setMenuBar() {
      ...
      JMenuItem newProject = new JMenuItem("Novo Projeto", icNewProject);
      newProject.setMnemonic(KeyEvent.VK_N);
      newProject.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_N, ActionEvent.CTRL_MASK));
      newProject.getAccessibleContext().setAccessibleDescription("Botão para criar um novo projeto");

      JMenuItem openProject = new JMenuItem("Abrir Projeto", icOpenProject);
      openProject.setMnemonic(KeyEvent.VK_O);
      openProject.getAccessibleContext().setAccessibleDescription("Botão para abrir um projeto existente");

      JMenuItem exitButton = new JMenuItem("Sair", KeyEvent.VK_E);
      exitButton.addActionListener(actionListener -> System.exit(0));

      JMenuItem saveProjectButton = new JMenuItem("Salvar Projeto", KeyEvent.VK_S);
   ...
   private void setFooterPanel() {
      footerPanel = new FooterPanel(font);
   }
   ...
   public void showOpenProjectDialog() {
      Platform.runLater(() -> {
         FileChooser.ExtensionFilter extentionFilter = new FileChooser.ExtensionFilter("Arquivos ExamClipper (*.examclipper-project)", "*.examclipper-project");
   ...
   public void showOpenPDFDialog() {
      Platform.runLater(() -> {
         FileChooser.ExtensionFilter extentionFilter = new FileChooser.ExtensionFilter("Arquivos PDF (*.pdf)", "*.pdf");
   ...
   private void createNewProject() {
      footerPanel.setLabelMessage("Criando Projeto...");
      footerPanel.show();
   ...
   private void loadProject(File file) {
      footerPanel.setLabelMessage("Abrindo Projeto...");
      footerPanel.show();
   ...
   private void saveProject() {
      ...
      footerPanel.setLabelMessage("Salvando Projeto...");
      footerPanel.show();
   ...
   private void convertPDF(File pdf) throws IOException {
      ...
      footerPanel.setLabelMessage("Carregando e Convertendo PDF...");
      footerPanel.show();
   ...
   private class LoadPagesTask extends SwingWorker<Void, Void> {
      ...
      protected Void doInBackground() throws Exception {
         ...
         while (conversor.isAlive()) {
            ...
            footerPanel.setValue(percent);
            footerPanel.setMessage(percent + " %");
            try {
               Thread.sleep(50);
   ...
   private class OpenProjectTask extends SwingWorker<Void, Void> {
      ...
      protected Void doInBackground() throws Exception {
         ...
         while (op.isAlive()) {
            ...
            if (step == 2) {
               numbersOfSteps = op.numbersOfSteps;
            }
            int percent = (int) ((step / (float) numbersOfSteps) * 100);
            footerPanel.setValue(percent);
            footerPanel.setMessage(percent + " %");
   ...
   private class SaveProjectTask extends SwingWorker<Void, Void> {
      ...
      protected Void doInBackground() throws Exception {
         ...
         while (sp.isAlive()) {
            ...
            if (step == 1) {
               numbersOfSteps = sp.numbersOfSteps;
            }
            int percent = (int) ((step / (float) numbersOfSteps) * 100);
            footerPanel.setValue(percent);
            footerPanel.setMessage(percent + " %");
